...
- 사용자가 직접 정리한 O/X 문항 80개를 기준으로 seed를 완전히 교체했습니다.
- GENERAL, IT, SCIENCE, LIFE 카테고리별로 20문항씩 넣었습니다.
- 기존 O/X 형식과 제출/랭킹 흐름은 그대로 유지했습니다.

확인 내용:

- 백엔드 전체 테스트를 다시 통과했습니다.
- GENERAL 테스트는 새 seed 기준으로 20문항 위에 1개를 추가하는 구조라 21건으로 맞췄습니다.

### OX 문제 전면 교체

작업 내용:

- 기존 함정형 문항을 버리고, 더 짧고 직접적인 O/X 문장 세트로 전면 교체했습니다.
- 사용자가 예시로 준 스타일인 `돌고래는 물고기가 아니다` 같은 단문 사실문 중심으로 다시 구성했습니다.
- 카테고리별 10문항씩 유지하면서, 문제 문장 자체를 완전히 새 세트로 바꿨습니다.

확인 내용:

- O/X 선택 구조는 그대로 유지됩니다.
- 백엔드 전체 테스트를 다시 통과했습니다.
...
- 기존 OX 문항이 너무 정직하게 느껴져서, 더 헷갈리는 표현으로 다시 손봤습니다.
- 일상 오해, 기술 상식, 과학 오해를 섞어서 문장 자체의 함정을 강화했습니다.
- 새 seed는 기존 데이터셋을 덮어쓰는 별도 마이그레이션으로 추가했습니다.

확인 내용:

- 문제 수는 카테고리별 10문항씩 유지됩니다.
- O/X 형식과 결과/랭킹 흐름은 그대로 유지됩니다.

### OX 함정 퀴즈 전환

작업 내용:

- 퀴즈 콘텐츠를 3지선다 상식형에서 OX 함정형으로 전환했습니다.
- `quiz_questions`에는 사실 여부를 판단하는 문장을 넣고, `quiz_choices`는 `O`와 `X` 두 개만 사용하도록 바꿨습니다.
- 프론트 퀴즈 화면에는 `O 또는 X로 판단하세요.` 안내를 추가했습니다.
- 기존 로컬 DB가 이전 마이그레이션 체크섬을 기억하고 있어서, 개발 환경에서는 Flyway 검증을 잠시 완화했습니다.

확인 내용:

- 새 seed는 `GENERAL`, `IT`, `SCIENCE`, `LIFE` 카테고리별로 10문항씩 제공합니다.
- 기존 퀴즈 제출/랭킹/로그인 흐름은 그대로 유지됩니다.

### 함정 퀴즈 콘텐츠 전환

작업 내용:

- 퀴즈 시드 문제를 단순 상식형에서 헷갈리는 함정형 문항으로 다시 구성했습니다.
- 프론트엔드 문구도 상식 퀴즈보다 함정 퀴즈에 가까운 표현으로 바꿨습니다.
- 카테고리 설명, 시작 문구, 결과 문구, 랭킹 문구를 함께 조정했습니다.

확인한 내용:

- 퀴즈 문제는 카테고리별 10문제 이상 유지됩니다.
- 백엔드 전체 테스트는 통과했습니다.
