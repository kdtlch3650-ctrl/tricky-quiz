...
이 프로젝트의 프론트엔드는 GitHub Pages로 배포할 수 있다.

- 배포 주소: https://kdtlch3650-ctrl.github.io/tricky-quiz/
- 배포 방식: GitHub Actions
- 배포 범위: 프론트엔드 정적 파일

GitHub Pages 배포본은 백엔드에 직접 연결되지 않으므로 데모 모드로 동작한다.
...
1. 위 배포 주소로 접속한다.
2. 데모 모드 배너가 보이는지 확인한다.
3. 홈 화면, O/X 퀴즈, 결과, 랭킹이 샘플 데이터로 표시되는지 확인한다.
4. 실제 서버 연동이 필요하면 로컬 실행 또는 별도 백엔드 배포를 사용한다.
